...
RUN ../configure --enable-languages=c,c++ --disable-multilib --prefix=/opt/gcc-{{ major_version }} && \
    make -j"$(nproc)" && \
    make install-strip
WORKDIR /
RUN rm -rf /tmp/gcc
...
COPY --from=compiler_stage /opt/gcc-{{ major_version }} /opt/gcc-{{ major_version }}

ENV CC=/opt/gcc-{{ major_version }}/bin/gcc
ENV CXX=/opt/gcc-{{ major_version }}/bin/g++

RUN ln -sf /opt/gcc-{{ major_version }}/bin/gcc /usr/bin/gcc-{{ major_version }} && \
    ln -sf /opt/gcc-{{ major_version }}/bin/g++ /usr/bin/g++-{{ major_version }} && \
    update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-{{ major_version }} 100 \
    --slave /usr/bin/g++ g++ /usr/bin/g++-{{ major_version }}

ENV PATH=/opt/gcc-{{ major_version }}/bin:$PATH
{% endmacro %}
